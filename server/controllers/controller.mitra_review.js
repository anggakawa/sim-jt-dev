...
module.exports = {
  ...
  async getMitraOrderList(req, res) {
    try {
      ...
      ON ov.order_id = o.order_id INNER JOIN users u ON u.user_id = ov.vendor_id AND ov.replaced = 0`);
      res.json(result);
    } catch (error) {
      console.log(error);
      res.json({
  ...
  async getMitraTimeline(req, res) {
    try {
      ...
      FROM order_logs o INNER JOIN users u ON u.user_id = o.user_id
        ...
        INNER JOIN order_vendor_history ov ON ov.order_id = i.order_id
        WHERE u.role_id = 3 AND (i.open_status = 2 OR i.open_status = 3) AND ov.replaced = 0`);
      res.json(result);
    } catch (error) {
      console.log(error);
      res.json({
